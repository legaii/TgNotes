...
class MongoPersistenceTest(IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.persistence = MongoPersistence(mongo_db_mock)

    async def test_user_data(self):
        user_data = UserData()
        user_data_json = {'user_data': user_data.to_json()}
        user_id_json = {'user_id': 0}
        MagicMock.find = MagicMock(return_value=[user_id_json | user_data_json])
        MagicMock.update_one = MagicMock()
        MagicMock.delete_one = MagicMock()

        await self.persistence.update_user_data(0, user_data)
        MagicMock.update_one.assert_called_once_with(user_id_json, {'$set': user_data_json}, upsert=True)

        self.assertIsNone(await self.persistence.refresh_user_data(0, user_data))

        all_data = await self.persistence.get_user_data()
        MagicMock.find.assert_called_once_with()
        ...
        MagicMock.delete_one.assert_called_once_with(user_id_json)

    async def test_conversations(self):
        key_json = {'key': [1, 2]}
        state_json = {'state': 3}
        MagicMock.find = MagicMock(return_value=[key_json | state_json])
        MagicMock.update_one = MagicMock()

        await self.persistence.update_conversation('a', (1, 2), 3)
        MagicMock.update_one.assert_called_once_with(key_json, {'$set': state_json}, upsert=True)
